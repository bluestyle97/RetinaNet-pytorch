...
def generate_anchors(stride, ratio_vals, scales_vals):
    """
    Generate anchors coordinates from scales and ratios.
    """
...
def snap_to_anchors(target, size, stride, anchors, num_classes, device):
    """
    Snap target boxes (x, y, w, h) to anchors.
    :param target: the ground truth of an image (no batch axis), [num_boxes, 5]
    :param size: the size of image, [w, h] not [h, w]!!!
    :param stride: the stride of feature map
    :anchors: the reference coordinates of anchors, [A, 4]
    """
    ...
    boxes, classes = target.split(4, dim=1)

    # generate anchors on every pixel of feature map
    ...
    N = anchors.size()[0]   # total anchors number
    M = boxes.size()[0]     # total boxes number
    xy1 = torch.max(anchors[:, :2].unsqueeze(1).expand(N, M, 2),
                    boxes[:, :2].unsqueeze(0).expand(N, M, 2))
    xy2 = torch.min(anchors[:, 2:].unsqueeze(1).expand(N, M, 2),
                    boxes[:, 2:].unsqueeze(0).expand(N, M, 2))
    inter = torch.prod((xy2 - xy1 + 1).clamp(0), 2)
    boxes_area = torch.prod(boxes[:, 2:] - boxes[:, :2] + 1, 1).unsqueeze(0).expand(N, M)
    anchors_area = torch.prod(anchors[:, 2:] - anchors[:, :2] + 1, 1).unsqueeze(1).expand(N, M)
    overlap = inter / (anchors_area + boxes_area - inter)

    # keep best box per anchor
    overlap, indices = overlap.max(dim=1)
    ...
    cls_target.scatter_(1, classes, 1)      # label to one-hot vector
...
def decode(all_cls_pred, all_box_pred, stride=1, threshold=0.05, topn=1000, anchors=None):
    """
    Decoding boxes from net's output and filtering.
    """
    # if torch.cuda.is_available():
    #     return decode_cuda(all_cls_pred.float(), all_box_pred.float(),
    #                        anchors.view(-1).tolist(), stride, threshold, top_n)
    device = all_cls_pred.device
    anchors = anchors.to(device).type(all_cls_pred.type())
    num_anchors = anchors.size()[0] if anchors is not None else 1
    num_classes = all_cls_pred.size()[1] // num_anchors
    height, width = all_cls_pred.size()[-2:]

    batch_size = all_cls_pred.size()[0]
    out_scores = torch.zeros((batch_size, topn), device=device)
    out_boxes = torch.zeros((batch_size, topn, 4), device=device)
    out_classes = torch.zeros((batch_size, topn), device=device)

    # Per item in batch
    for batch in range(batch_size):
        cls_pred = all_cls_pred[batch, :, :, :].contiguous().view(-1)
        box_pred = all_box_pred[batch, :, :, :].contiguous().view(-1, 4)

        # Keep scores over threshold
        keep = (cls_pred >= threshold).nonzero().view(-1)
        if keep.nelement() == 0:
            continue

        # Gather top elements
        scores = torch.index_select(cls_pred, 0, keep)
        scores, indices = torch.topk(scores, min(topn, keep.size()[0]), dim=0)
        indices = torch.index_select(keep, 0, indices).view(-1)
        classes = (indices / width / height) % num_classes
        classes = classes.type(all_cls_pred.type())

        # Infer kept boxes
        x = indices % width
        y = (indices / width) % height
        a = indices / num_classes / height / width
        box_pred = box_pred.view(num_anchors, 4, height, width)
        boxes = box_pred[a, :, y, x]

        if anchors is not None:
            grid = torch.stack([x, y, x, y], 1).type(all_cls_pred.type()) * stride + anchors[a, :]
...
def nms(all_scores, all_boxes, all_classes, nms_threshold=0.5, ndetections=100):
    """
    ...
    for batch in range(batch_size):
        ...
        for i in range(ndetections):
            ...
            criterion = ((scores > scores[i]) |
                         (inter / (areas + areas[i] - inter) <= nms_threshold) |
                         (classes != classes[i]))
            criterion[i] = 1
...
if __name__ == '__main__':
    anchors = generate_anchors(8, [1.0, 2.0, 0.5], [4 * 2**(i/3) for i in range(3)])
    print(anchors)
    size = (1200, 1000)
    x, y = torch.meshgrid([torch.arange(0, size[i], 64, dtype=torch.float32) for i in range(2)])
    xyxy = torch.stack((x, y, x, y), 2).unsqueeze(0)
    ...
    overlap = torch.rand(10, 5)
    overlap, indices = overlap.max(dim=1)
    print(overlap.size(), indices.size())
    print(overlap)
    print(indices)
